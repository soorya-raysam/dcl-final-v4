...
def get_live_health_data():
    """Fetch system health data via SSH with dynamic IP/password."""
    ...
    try:
        if request.method == "POST":
            ...
            result = generate_health_data(ip=ip, password=password)
                        ...
                        # ✅ Log each executed command returned from generate_health_data
            try:
                ...
                if logs:
                    for cl in logs:
                        ...
                        # Excel path isn't tracked yet, so leave None
        ...
        else:  # Handle GET fallback
            print("⚠️ GET request detected — using cached or default data.")
            result = (
                _cached_health_data
                if _cached_health_data
                else generate_health_data(ip="172.16.70.39", password="Avaya@123")
            )
...
def get_list_trunk_group_data():
    """Return parsed list trunk-group data as JSON."""
    try:
        # Find latest Excel file
        excel_files = sorted(
            glob.glob(os.path.join(REPORT_DIR, "list_trunk-group_*.xlsx")),
        ...
        return jsonify({"status": "success", "data": data, "excel_path": latest_excel})
...
@app.route("/get-yesterday-peak-data", methods=["GET", "POST"])
def get_yesterday_peak_data():
    ...
    try:
        ...
        # 1. Run the Avaya command and parse output directly
        output = run_avaya_command()
        df = parse_trunk_summary(output)

        # 2. Save Excel (for download) but keep data in memory
...
def get_list_measurements_outage_trunk_last_hour():
    ...
    try:
        ...
        # ✅ Run the actual Avaya command
        output = run_avaya_command("list measurements outage-trunk last-hour")

        # # --- Optional debug ---
        # print("\n=== DEBUG OUTAGE COMMAND OUTPUT ===")
        # for i, line in enumerate(output.splitlines()[:40], 1):
        #     print(f"{i:02d}: {line}")
        # print("=== END DEBUG ===\n")

        # ✅ Find all data lines (start with a number)
        # ✅ Find all data lines (start with a number)
        data_lines = re.findall(r"(?m)^\s*\d+\s+.*", output)

        # 🔧 Filter out footer/prompt lines like:
        # "press CANCEL to quit" or "press NEXT PAGE to continue"
        filtered_lines = []
        ...
        for line in filtered_lines:
            ...
            tokens = (tokens + [""] * 6)[:6]  # pad/trim to 6 cols
